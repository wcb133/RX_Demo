...
class DriverVC: UIViewController {
    ...
    let inputTF1:UITextField = UITextField()
    
    let messageLab:UILabel = UILabel()
...
//        dealWithDriver()
    ...
    func dealWithDriver()  {
        let result = inputTF.rx.text.orEmpty.asDriver().debounce(RxTimeInterval.milliseconds(1)).flatMapLatest {  return self.dealwithData(inputText: $0).asDriver(onErrorJustReturn: "检测到了错误事件") }
    ...
    func dealWithoutDriver()  {
        ...
        let result = inputTF.rx.text.skip(1).flatMap { [weak self](input) -> Observable<Any> in
            return (self?.dealwithData(inputText: input ?? ""))!.observeOn(MainScheduler.instance).catchErrorJustReturn("发生错误")
        ...
        result.subscribe { (even) in
            ...
            case .error(let error)://这个方法不会走的，错误已经被catchErrorJustReturn处理了，一旦走了error或者是completed，那这个信号就无法在发出信号了
                print("发生错误 ===== \(error)")
            case .completed:
                print("完成 ===== ")
            }
